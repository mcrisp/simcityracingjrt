...
function scroll_to_selected_idxjs() {
    if (autoscroll_cond() && selected_idxjs_scrollpos != -1 && ( f3_box || (group_by_class == 0 && group_nb_leading_cars == 0 && group_reverse_mode == 0)) ) {  // REM: on doit gérer l'autoscroll différemment si on groupe les classements par classe ou bien lors d'un affichage en 'reverse', ou un autre type de regroupement avec group_nb_leading_cars
        ...
        if (f3_box == 0 && filtered == false && filter_colorized == 0) {
            ...
            if (group_by_class == 0 && group_nb_leading_cars > 0) {
