...
function responsive_dim() {
    ...
    var decale_clutch = 0;
    if (disp_clutch) {
        document.getElementById("clutch_cont").style.display = "block";
        document.getElementById("clutch_text").style.display = "block";
        decale_clutch = 25;
    } else {
        document.getElementById("clutch_cont").style.display = "none";
        document.getElementById("clutch_text").style.display = "none";
        decale_clutch = 0;
    }


    set("t_cont", 640-225, 1000 - decale_clutch, 450, 80, 0.032);
    set("b_cont", 640-225, 1000 + 85 - decale_clutch, 450, 80, 0.032);
    set("t_text", 640-225, 1000 + 2 - decale_clutch, 450, 80, 0.052);
    set("b_text", 640-225, 1000 + 85 + 2 - decale_clutch, 450, 80, 0.052);
    ...
    if (disp_clutch) {
        set("clutch_cont", 640 - 225, 1000 + 85 + 85 - decale_clutch, 450, 80, 0.032);
        set("clutch_text", 640 - 225, 1000 + 85 + 85 + 2 - decale_clutch, 450, 80, 0.052);
        $("#clutch").css("border-radius", 15 * w / compteur_ref_w + "px");
        $("#clutch_cont").css("border-radius", 15 * w / compteur_ref_w + "px");
    }
    ...
    if ((carname in car_with_ers_drs) || (carname in car_with_drs)) {
        ...
        for (var s = 1; s <= 9; s += 1) {
            if (s%2 == 0) {
                ...
                rpm_bg_context.lineWidth = rayon_compteur / 60 / 2;
            }
            rpm_bg_context.beginPath();
            x = Math.floor(1320 * w / compteur_ref_w);
            y = Math.floor((300 + 680*s/10) * w / compteur_ref_w);
            x2 = Math.floor((1320 + 110) * w / compteur_ref_w);
            y2 = Math.floor((300 + 680*s/10) * w / compteur_ref_w);
