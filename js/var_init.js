...
function change_class_selected() {
    ...
    // Pour le 'group by class' si on veut définir manuellement 'my own class'
    my_own_class_in_url = getParamValue('myownclassid');
    if (my_own_class_in_url === false || isNaN(parseInt(my_own_class_in_url))) {
        my_own_class_in_url = -1;  // pour signifié qu'aucune classe n'est sélectionnée
    } else {
        my_own_class_in_url = parseInt(my_own_class_in_url);
    }
...
function init_var(create_canvas_trackmap) {
    ...
    selected_idxjs = -1;
    selected_idxjsold = 1;
    selected_idxjs_in_init_group_by_class = -1;
    my_own_class_in_url = -1;
    lastlap = [];
    bestlap = [];
    besttag = 0;
    lasttag = 0;
...
//opt_colorize_drivers_defined = 0;
...
update_aff_tstamp = 0;  // sert à savoir quand on a chargé le update_aff et ainsi éviter le le recharger plusieurs fois d'affilés
draw_track_change_classid_done_tstamp = 0;

reload_clublogos = {};
reload_carlogos = {};

new_session_tstamp_old = -1;