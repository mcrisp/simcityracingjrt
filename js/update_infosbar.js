function update_infosbar() {
    ...
    // On affiche la valeur 'fuel to add' entrée manuellement ou bien ll'offset en mode auto pendant 2 secondes
    if (refuel_mode == 1) {
        nb_decimal_fnman = 2;
        ...
        if (conso > 0.01) {
            if (donnees.pro_v == 1 || donnees.try_v == 1 || donnees.pro_v == undefined) {
                if (fuelfactor * coef_fuel * fuelneed > 9999) {
                    ...
                    set_inner_html("fuelneed", (fuelfactor * coef_fuel * fuelneed).toFixed(1));
                }
            } else {
            ...
            for (var c in donnees.classes) {
                if (donnees.carclasscolor != undefined && (c in donnees.carclasscolor)) {
                    if (donnees.nbcars_class[c] > 0 && donnees.sof[c] != undefined) {
                        ...
                        a2 = "";
                        if (f3_box == 0) {
                            a1 += "<div onmousedown='class_selected=" + c + ";init_group_by_class();update_datas_text_save();'";
                            a2 += "<div onclick='class_selected=" + c + ";init_group_by_class();update_datas_text_save();'";
                        } else {
                            a1 += "<div ";
                            a2 += "<div ";
                        }
