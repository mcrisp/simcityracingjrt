    ...
    if (donnees.d != undefined) {
        for (i in donnees.d) {
            ...
            if (Date.now() - click_status.start > focus_replay_delay && click_status.caridx == i) {  // si click long sur le pilote on affiche la ligne en rouge
                ...
                if (i == selected_idxjs && selected_driver_mode != 0 && fond_blanc != 3) {
                    ...
                    if (donnees.d[i].fr == 0) { // Si les positions ne sont pas gelées
                        ...
                        tmp_bg = Math.floor(line_num[i]) % 2 == 1 ? 'rgba(51,51,51,' + 0.5 * transparence_lignes + ')' : 'rgba(34,34,34,' + 0.5 * transparence_lignes + ')';
                        ...
                        tmp_bg = Math.floor(line_num[i]) % 2 == 1 ? 'rgba(210,210,210,' + 1 * transparence_lignes + ')' : 'rgba(200,200,200,' + 1 * transparence_lignes + ')';
                        //document.getElementById('p' + i).style.backgroundColor = tmp_bg;
                        set_style_bg('p' + i, tmp_bg);
                    }
                }
            ...
            if (position_change_animation) {
                ...
                //if (opt_colorize_drivers_defined && !document.getElementById('opt_colorize_drivers_' + i).checked) {
            ...
            if (show_cars_stopped_darker && donnees.d[i].dp == -4 && mouse_over_idx != i) { // Si le pilote n'est pas encore sur la grille
                ...
                if (show_cars_stopped_darker && mouse_over_idx != i && (donnees.d[i].fr == 0) && (donnees.d[i].ts == -1 || donnees.d[i].s < 1)) { // Display name grey when driver is deconnected temporary or not or when He is stop anywhere
            ...
            if (donnees.d[i].pr == 1) {
                ...
                tmp_col = '#000000';
                tmp_bg = '#ff9900';
                if (donnees.d[i].rt == donnees.d[i].st && donnees.d[i].st != 0) {  // cela signifie que c'est un tow
                    tmp_bg = '#ff0000';
                    tmp_col = '#ffffff';
                }
                ...
                if (show_name_orange_inpits) {
                    if (donnees.d[i].rt == donnees.d[i].st && donnees.d[i].st != 0) {  // cela signifie que c'est un tow
                        tmp_name_color = '#ff0000';
                    } else {
                        tmp_name_color = '#ff9900';
                    }
                } else {
            ...
            if (i == selected_idx && show_name_selected_orange) {
                //tmp_name_color = "#f5b326";
                tmp_name_color = show_name_selected_color;
            }

            if (tmp_name_color != "") {
                set_style_color('name' + i, tmp_name_color);
            }
            ...
            if (donnees.d[i].ps == 1) {
                if (donnees.d[i].tow == 0 || donnees.d[i].tow == undefined) {
                    ...
                    if (donnees.d[i].rt == donnees.d[i].st && donnees.d[i].st != 0) {  // cela signifie que c'est un tow
                        set_style_bg('pitstalltime' + i, '#ff0000');
                    } else {
