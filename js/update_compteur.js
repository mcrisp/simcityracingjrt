...
function aff_rpm() {
    ...
    degrade1.addColorStop(0, hexToRGBA(compteur_rev1_color, 0));
    //degrade1.addColorStop(1,"rgba(0,0,255,1)");
    degrade1.addColorStop(1, hexToRGBA(compteur_rev1_color, 1));
    ...
    // On met à jour quand le max_rpm change
    if (max_rpm != max_rpm_old) {
        responsive_dim();
    }
    ...
    if (rpm_angle >= red_angle) {
        ...
        degrade2.addColorStop(0, hexToRGBA(compteur_rev2_color, 0));
        //degrade2.addColorStop(1, "rgba(255,0,0,1)");
        degrade2.addColorStop(1, hexToRGBA(compteur_rev2_color, 1));
...
function update_compteur() {
    ...
    if (donnees.typ <= 33) {
        ...
        if (donnees.clutch != undefined) {
            document.getElementById("clutch").style.right = (donnees.clutch) + "%";
            if (donnees.clutch == 100) {
                document.getElementById("clutch_text").style.color = "#000000"
            } else if (donnees.clutch == 0) {
                document.getElementById("clutch_text").style.color = "#006cd8"
            } else {
                document.getElementById("clutch_text").style.color = "#ffffff"
            }
        }
