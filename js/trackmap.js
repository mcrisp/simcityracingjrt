...
// On bloque aussi l'autoscroll si on scroll down jusqu'à ce qu'on scroll up
function change_turn_edit_ldp(event) {
    ...
    if (event != undefined) {
        ...
        if (d > 0 && turn_edit == 0 && autoscroll != 0 && group_by_class == 0 && f3_box == 0) {  // REM : on ne peut pas désactiver l'autoscroll en mode f3 box
            if (autoscroll_initial == -1) autoscroll_initial = autoscroll;
            autoscroll = 0;  // on désactive l'autoscroll
        ...
        if (d < 0 && turn_edit == 0 && autoscroll != 1 && group_by_class == 0) {
            if (autoscroll_initial == 1) {
                autoscroll = 1;  // on réactive l'autoscroll s'il était activé initialement
                if ( text_save_typ1 != -1 && donnees_new == null ) {  // si on est hors connexion
                    document.getElementById("events_ticker").innerHTML = "";
                    update_datas(text_save_typ1);
                } else {
...
function trackmap() {
    ...
    if (donnees.d != undefined) {
            ...
            if (donnees.d != undefined && i in donnees.d) {
                ...
                if ((donnees.d[i].fr == 0) && (donnees.d[i].pr == 1 || donnees.d[i].s > 1 || donnees.d[i].cts != -1) && turn_edit == 0 && north_edit == 0) {
                    ...
                    if (donnees.d[i].cts != -1 || donnees.d[i].pr != 1) {  // si un pilote est aux pits et a quitté le jeu on ne l'affiche pas
                        ...
                        if (donnees.d[i].s < 40 && !(donnees.d[i].pr)) {  // Les pilotes quasiments arrêtés sur la piste on les met en jaune
        ...
        trackmap_context.globalAlpha = trackmap_car_color_transparency;
        pace_car_on_trackmap("#ffff00", 1, 1, 1);
        trackmap_context.globalAlpha = 1;
        // on dessine un bord autour du pace car
        tmp_trackmap_car_border_disp = trackmap_car_border_disp;
        tmp_trackmap_car_border_color = trackmap_car_border_color;
        tmp_trackmap_car_border_coef = trackmap_car_border_coef;
        if (tmp_trackmap_car_border_disp) {
            trackmap_context.globalAlpha = 1;
            tmp_r = 1 - 0.1 * tmp_trackmap_car_border_coef;
            pace_car_on_trackmap(tmp_trackmap_car_border_color, tmp_r, 0, 7 / 10 * 2 / tmp_r * tmp_trackmap_car_border_coef * 1);
        }
...
function pace_car_on_trackmap(coul, taille, plein, epaisseur_trait) {

    // On affiche le pace car que si sa vitesse est non nulle
    if ( donnees.pace_car_ldp != undefined && donnees.pace_car_speed != undefined && donnees.pace_car_speed != 0 ) {

        ldp = donnees.pace_car_ldp;
        ...
        if (k in track.x && k in track.y && donnees.k_max > 0) {
            ...
            if (plein != 0 && taille == 1) {
                ...
                trackmap_context.fillText("SC", x, y + 0.5 * rayon * tmp_trackmap_carnum_coef);
...
function driver_on_trackmap(coul, ldp, caridx, taille, plein, epaisseur_trait) {
    ...
    var is_myclass = false;
    if (donnees.me in donnees.d && donnees.d[donnees.me].classid == donnees.d[caridx].classid) {
        is_myclass = true;
    }
...
function draw_fleche_start_finish() {
    ...
    if(dessine_ok) {
        ...
        if (trackmap_start_finish_line_disp == 1) {
            ...
            coord_x_y = calc_x_y(ldp_decal, 0);
            x0 = coord_x_y[0];
            y0 = coord_x_y[1];
            coord_x_y = calc_x_y(ldp_decal, 1);
            x2 = coord_x_y[0];
            y2 = coord_x_y[1];
...
function draw_turn(turn, ldp, side, info) {
    ...
    if (l != 0) {
        ...
        trackmap_fond_turns_context.font = "bold " + trackmap_turn_num_coef * 2 * rayon + "px " + "'" + trackmap_font_family + "'";
        ...
        if (trackmap_circular == 0) {  // on n'affiche pas les noms des virage en mode trackmap circulaire
            ...
            trackmap_fond_turns_context.font = "bold " + trackmap_turn_info_coef * 1 * rayon + "px " + "'" + trackmap_font_family + "'";
...
function draw_turn_edit(coul, turn, ldp, side, info) {
    if (trackmap_loaded) {
        ...
        if (l != 0) {
            ...
            trackmap_context.fillStyle = coul;
            trackmap_context.strokeStyle = coul;
            trackmap_context.font = "bold " + trackmap_turn_num_coef * 2 * rayon + "px " + "'" + trackmap_font_family + "'";
            trackmap_context.textAlign = "center";
            trackmap_context.fillText(turn, x, y + rayon / 1.5);

            trackmap_context.fillStyle = coul;
            trackmap_context.font = "bold " + trackmap_turn_info_coef * 1 * rayon + "px " + "'" + trackmap_font_family + "'";
...
function draw_winddir(north, winddir) {
    ...
    trackmap_context.fillStyle = "#0077ff";
    trackmap_context.font = "bold " + 1.7 * rayon2 + "px " + "'" + trackmap_font_family + "'";
    trackmap_context.textAlign = "left";
